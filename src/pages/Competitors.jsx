...
import { Plus, Trash2, BarChart2, Loader2, TrendingUp, TrendingDown, Target, Lightbulb, ChevronDown, ChevronUp, Search } from 'lucide-react';
...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div className="glass-card rounded-xl px-3 py-2 border border-white/10 text-xs">
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color }}>{p.name}: {p.value}/100</p>
      ))}
    </div>
  );
}
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm" style={{ background: color }}>
            {data.handle.replace('@', '').replace('https://', '').slice(0, 2).toUpperCase()}
          </div>
          <div>
            <p className="font-semibold text-foreground">{data.handle}</p>
            <p className="text-xs text-muted-foreground">{data.niche}</p>
          </div>
        </div>
...
export default function Competitors() {
  ...
  const addHandle = () => {
    ...
    if (handles.length >= 5) {
      setError('Maximum 5 competitors at a time');
      return;
    }
...
- strategy: 2-3 sentence summary of their overall brand strategy — what approach are they using to grow? What makes their brand recognizable?
...
- how_to_beat: 3 actionable tips for a competitor to outperform them (each 1 sentence, be specific).
- top_content_types: Array of 3-5 content formats they use most (e.g. "Reels", "Carousels", "Stories", "Long-form video", "Threads").
- posting_frequency: Estimated posts per week (e.g. "3-4 per week").`,
    ...
    <div className="p-6 lg:p-10 max-w-5xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="glass-card rounded-2xl p-6 neon-border mt-6 mb-6">
          ...
          <p className="text-xs text-muted-foreground mb-3">Enter their @handle or profile URL (up to 5)</p>
          ...
          <Button onClick={runAnalysis} disabled={loading || handles.length === 0} className="gap-2 neon-glow">
            {loading ? (
              <>
                ...
                Analyzing {handles.length} competitor{handles.length > 1 ? 's' : ''}...
              </>
            ) : (
              <>
                <Search className="w-4 h-4" />
                {analysisData ? 'Re-analyze' : 'Run Competitor Analysis'}
              </>
            )}
          </Button>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left py-2 pr-4 text-xs text-muted-foreground font-semibold uppercase tracking-wide">Competitor</th>
                    <th className="text-center py-2 px-2 text-xs text-muted-foreground font-semibold uppercase tracking-wide">Brand Score</th>
                    <th className="text-center py-2 px-2 text-xs text-muted-foreground font-semibold uppercase tracking-wide">Engagement</th>
                    <th className="text-center py-2 px-2 text-xs text-muted-foreground font-semibold uppercase tracking-wide">Growth</th>
                    <th className="text-center py-2 px-2 text-xs text-muted-foreground font-semibold uppercase tracking-wide">Content</th>
                    <th className="text-center py-2 px-2 text-xs text-muted-foreground font-semibold uppercase tracking-wide">Consistency</th>
                    <th className="text-center py-2 px-2 text-xs text-muted-foreground font-semibold uppercase tracking-wide">Posts/Week</th>
                  </tr>
                </thead>
                <tbody>
                  {analysisData.map((d, i) => (
                    <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-foreground">{d.handle}</span>
                        </div>
                      </td>
                      <td className="py-3 px-2"><div className="flex justify-center"><ScoreBadge score={d.brand_score} /></div></td>
                      <td className="py-3 px-2"><div className="flex justify-center"><ScoreBadge score={d.engagement} /></div></td>
                      <td className="py-3 px-2"><div className="flex justify-center"><ScoreBadge score={d.audience_growth} /></div></td>
                      <td className="py-3 px-2"><div className="flex justify-center"><ScoreBadge score={d.content_quality} /></div></td>
                      <td className="py-3 px-2"><div className="flex justify-center"><ScoreBadge score={d.consistency} /></div></td>
                      <td className="py-3 px-2 text-center text-muted-foreground text-xs">{d.posting_frequency || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
